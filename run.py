...
def configure_postscreen():
    ...
    check_call(
        [
            "postconf",
            "-e",
            "postscreen_dnsbl_sites = zen.spamhaus.org*2 bl.spamcop.net*1 b.barracudacentral.org*1 swl.spamhaus.org*-3",
        ]
    )
...
def configure_opendkim():
    ...
    with open("/etc/opendkimKeyTable", "w") as key_table, open(
        "/etc/opendkimSigningTable", "w"
    ) as signing_table:
        for (domain, domain_info) in config["virtual_domains"].items():
...
def spawn_opendkim():
    ...
    check_call(["postconf", "-e", "smtpd_milters=unix:/run/opendkim/opendkim.sock"])
